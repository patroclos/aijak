...
void mm_init(uint32 kernel_end)
{
...
void pfree(void *mem)
{
	if ((uint32)mem < pheap_begin || (uint32)mem > pheap_end)
		return;
	// determine page
	uint32 ad = (uint32)mem;
	ad -= pheap_begin;
	ad /= 4096;
	// add has to id of the page
	pheap_desc[ad] = 0;
	return;
}
...
char *malloc(size_t size)
{
	...
	// loop through blocks and find block with same or bigger size
	uint8 *mem = (uint8 *)heap_begin;
	while ((uint32)mem < last_alloc)
	{
		alloc_t *a = (alloc_t *)mem;
		// if the alloc has no size we have reached the end of allocation
		if (!a->size)
			goto nalloc;

		// if the alloc has a status of 1 (allocated) then add its size
		// and the size of alloc_t to the memory and continue looking
		if (a->status)
		{
		...
		// if the alloc is not allocated and its size is bigger or eq
		// to requested size, then adjust its size, set status and ret location
		if (a->size >= size)
		{
			// set to allocated
			a->status = 1;
		...
		// if it isnt allocated, but the size is not good, then
